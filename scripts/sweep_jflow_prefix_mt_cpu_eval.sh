#!/usr/bin/env bash
# CPU Best-NT fixed; sweep bridge-flow M × prefix-T (T≠0), 25 ep/task.
# Artifacts: epoch*_m{M}_pt{T}.json / step_*_m*_pt*.DONE
set +e
set -uo pipefail
...
LOGROOT="${LOGROOT:-$REPO/nohup_logs/${RUN_GROUP}_mt_grid_cpu}"
...
launch_one() {
  ...
  local name out donef lock slot cpu0 cpu1 ptok
  ...
  log "LAUNCH ${name} step=${step} m=${m} pt=${pt} cpus=${cpu0}-${cpu1} ep=${EPISODES}"
  (
    set +e
    taskset -c "${cpu0}-${cpu1}" \
...
log "START CPU MT-grid group=${RUN_GROUP} Ms=${MS[*]} PTs=${PTS[*]} MAX=${MAX} ep=${EPISODES}"
...
while true; do
  ...
  if (( ${#pending[@]} > 0 )); then
    ...
    if (( ${#PIDS[@]} == 0 )); then
      log "SWEEP_COMPLETE CPU M×pt grid"
      break
    fi
  fi
  sleep "$POLL_SEC"
