#!/usr/bin/env bash
# PathBridger_AF pilot queue — action-free only (no sac/td3/50-50).
# Envs: aml / cube-double / puzzle-4x4 / scene
# Seeds: 0,1,2  |  online 250k  |  offline = algorithm defaults (1M/3M/50k)
# GPUs: 2 × 1 slot. Error-21 thread pins + taskset.
set +e
set -uo pipefail

REPO="${REPO:-/home/ext_csv/PathBridger_AF}"
PY="${PY:-/home/ext_csv/miniconda3/envs/offrl/bin/python}"
LOGROOT="${LOGROOT:-$REPO/nohup_logs/pilot_af}"
# shellcheck disable=SC2206
GPU_LIST=(${GPU_LIST:-0 1})
MAX="${MAX:-${#GPU_LIST[@]}}"
RUN_GROUP="${RUN_GROUP:-pilot_250k}"
...
launch_one() {
  local algo="$1" env="$2" cfg="$3" seed="$4"
  local tag out slot gpu cpu0 cpu1
...
log "START pilot AF-only jobs=${#QUEUE[@]} GPUs=${GPU_LIST[*]} MAX=${MAX} online=${ONLINE_STEPS} seeds=${SEEDS[*]}"
...
while (( idx < ${#QUEUE[@]} )) || (( ${#PIDS[@]} > 0 )); do
  ...
  while (( idx < ${#QUEUE[@]} )) && (( ${#PIDS[@]} < MAX )); do
    IFS='|' read -r algo env cfg seed <<<"${QUEUE[$idx]}"
    idx=$((idx + 1))
    launch_one "$algo" "$env" "$cfg" "$seed"
  done
