#!/usr/bin/env bash
# PathBridger PBF paper params + stochastic bridge (seed0): all 8 pbf configs.
# Path weighting OFF (beta=0). Compare to provenance PBF (deterministic bridge).
# Backends via PREFIX_MODEL / RUN_GROUP:
#   low_rank_gaussian → paper_s0_pbf_lrg  (default)
#   joint_flow        → paper_s0_pbf_jflow
# Paper PBF: endpoint=flow, h_a=5, batch=1024, 1M steps.
# Concurrency: 2 GPUs × 2 slots. Soft orch ATTACH. Error-21 pins + taskset.
#
# Training uses the batched-forward / shared-bridge hot path in
# agents/pathbridger.py (new LAUNCH/RESUME import from disk; ATTACH keeps the
# in-memory code of already-running trainers).
#
# CLI pins (do not rely on mutable defaults):
#   --agent.path_weight_beta=0
#   --agent.prefix_model=${PREFIX_MODEL}
# Eval: sample M=4 stochastic bridges per selected endpoint, pick via TransV chain:
#   --agent.eval_prefix_selection=transv_chain
...
case "$PREFIX_MODEL" in
  low_rank_gaussian) RUN_GROUP_DEFAULT=paper_s0_pbf_lrg ;;
  joint_flow) RUN_GROUP_DEFAULT=paper_s0_pbf_jflow ;;
  *) RUN_GROUP_DEFAULT="paper_s0_pbf_${PREFIX_MODEL}" ;;
esac
...
PATH_WEIGHT_BETA="${PATH_WEIGHT_BETA:-0.0}"
PATH_WEIGHT_WARMUP="${PATH_WEIGHT_WARMUP:-100000}"
PATH_WEIGHT_RAMP="${PATH_WEIGHT_RAMP:-100000}"
EVAL_PREFIX_SELECTION="${EVAL_PREFIX_SELECTION:-transv_chain}"
EVAL_NUM_PREFIX_SAMPLES="${EVAL_NUM_PREFIX_SAMPLES:-4}"
EVAL_INCLUDE_DETERMINISTIC_PREFIX="${EVAL_INCLUDE_DETERMINISTIC_PREFIX:-False}"
...
resolve_resume() {
  ...
  local es
  es="$(env_substr "$env")"
  "$PY" - "$EXP_ROOT" "$es" "$PREFIX_MODEL" <<'PY'
import re, sys
from pathlib import Path
root, es, pm = Path(sys.argv[1]), sys.argv[2], sys.argv[3]
if not root.is_dir():
    raise SystemExit(0)
needle = f"_{pm}"
cands = []
for d in root.iterdir():
    if not d.is_dir() or es not in d.name or needle not in d.name:
        continue
    ck = d / 'checkpoints'
    steps = []
...
find_live_train_pid() {
  local env="$1"
  local tag="pbf_stoch_${PREFIX_MODEL}_${env}_s0"
  ...
  while read -r pid; do
    [[ -z "$pid" ]] && continue
    cmd="$(ps -p "$pid" -o args= 2>/dev/null || true)"
    if [[ "$cmd" == *"configs/pbf/${env}.py"* && "$cmd" == *"${RUN_GROUP}"* \
          && "$cmd" == *"prefix_model=${PREFIX_MODEL}"* ]]; then
      echo "$pid"
      return 0
    fi
...
log "TRAIN queue ${#JOBS[@]} jobs; ${#GPUS[@]} GPUs × ${MAX_PER_GPU} slots; group=${RUN_GROUP} (pbf+stoch)"
log "PIN agent.path_weight_beta=${PATH_WEIGHT_BETA} prefix_model=${PREFIX_MODEL}"
log "PIN eval_prefix_selection=${EVAL_PREFIX_SELECTION} eval_num_prefix_samples=${EVAL_NUM_PREFIX_SAMPLES} include_det=${EVAL_INCLUDE_DETERMINISTIC_PREFIX}"
...
launch_one() {
  ...
  if [[ -n "${resume_info:-}" ]]; then
    ...
    log "RESUME gpu=${gpu} cpus=${cpu0}-${cpu1} slot=${slot} ${tag} from step=${restore_step}"
  else
    log "LAUNCH gpu=${gpu} cpus=${cpu0}-${cpu1} slot=${slot} ${tag} (fresh)"
  fi
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" \
    ...
    "$PY" -u main.py \
      ...
      --agent.prefix_model="${PREFIX_MODEL}" \
      --agent.eval_prefix_selection="${EVAL_PREFIX_SELECTION}" \
      --agent.eval_num_prefix_samples="${EVAL_NUM_PREFIX_SAMPLES}" \
      --agent.eval_include_deterministic_prefix="${EVAL_INCLUDE_DETERMINISTIC_PREFIX}" \
      "${restore_args[@]}" \
      >"$logf" 2>&1 &
  SLOT_PIDS[$slot]=$!
  SLOT_TAGS[$slot]="$tag"
...
while (( next_job < ${#JOBS[@]} )) || slots_busy; do
  for ((slot=0; slot<total_slots; slot++)); do
    ...
    if [[ -z "${SLOT_PIDS[$slot]:-}" ]] && (( next_job < ${#JOBS[@]} )); then
...
log "QUEUE_DONE group=${RUN_GROUP} prefix=${PREFIX_MODEL}"
