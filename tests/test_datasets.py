...
def test_batch_contract_and_close_goal_clip_pad(monkeypatch):
    ...
    assert batch["bridge_targets"].shape == (2, 5, 2)
...
def test_every_sampled_index_stays_inside_its_source_episode():
    ...
    bridge_target_ids = _state_ids(batch["bridge_targets"])
    expected = np.minimum(
        starts[:, None] + np.arange(1, 6, dtype=np.int64)[None, :],
        endpoint_target_ids[:, None],
    )
    np.testing.assert_array_equal(bridge_target_ids, expected)
...
def test_random_actor_goal_keeps_endpoint_target_on_source_trajectory(monkeypatch):
    ...
    np.testing.assert_array_equal(
        _state_ids(batch["bridge_targets"]),
...
def test_observation_state_scale_uses_training_std_and_floor():
    dataset = Dataset.create(
        observations=np.asarray(
            [[0.0, 3.0, 7.0], [2.0, 3.0, 7.0]],
            dtype=np.float32,
        ),
        actions=np.zeros((2, 1), dtype=np.float32),
    )
    scale = observation_state_scale(dataset, floor=0.25)
    np.testing.assert_allclose(scale, [1.0, 0.25, 0.25], atol=1e-6)
